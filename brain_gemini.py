...
def run_ai_analysis(law, qnet_certs_text, attempt_count=5):
    ...
    #### ⭐ 우대분류 (메인 분류 — 가장 먼저 판단)
    해당 조문이 국가기술자격 취득자에게 주는 우대가 다음 5가지 중 무엇인지 1개로 지정하세요.
    한 조문에 여러 우대가 섞이면 아래 '우선순위'가 높은 것 하나로 지정합니다.
    우선순위: 의무고용 > 직무권한부여 > 인사우대 > 시험면제 (이 순서로 우선)
    ...
    ※ 자격과 무관하거나 우대가 없으면 "해당없음"으로 판별하고 우대분류는 "기타"로 둡니다.
