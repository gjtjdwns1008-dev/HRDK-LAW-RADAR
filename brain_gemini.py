...
def run_ai_analysis(law, qnet_certs_text, attempt_count=5):
    ...
    ---
    위 [분석 대상 법령]이 다음 국가기술자격 종목 중 어느 것과 연관되는지 파악하십시오.
    [국가기술자격 종목 리스트]
    {qnet_certs_text}
    ...
    * 1차 축 (법령의 취급):
      ...
      - Z. 제외 (위 A~E 어디에도 해당하지 않음. 자격을 직접 취급하는 조항이 없어 정책 충돌 위험이 없는 경우. 단순관련/해당없음 법령은 대부분 여기에 해당)
    * 2차 축 (위험도 - 선경력을 요구하는 경력이음형과 충돌하는 정도):
      ...
      - N (무관): 직역 진입 자체를 막지 않는 단순 부가우대 (D, E 유형에 해당).
      - X (해당없음): 취급유형이 Z(제외)인 경우. ★중요: 취급유형이 Z이면 위험도는 반드시 X입니다 (Z↔X는 항상 짝). 자격을 직접 취급하는 조항이 없어 위험도 판단 자체가 무의미한 경우.

    #### Track 2. 국민(구직자) 관점 : 노동시장 효용 (대국민 알림용)
    구직자 입장에서 "이 자격증을 따면 취업에 어떤 구체적인 이득이 있는가?"를 11개 세부 유형으로 분류하세요.
    한 조항은 반드시 한 칸에만 속하며, 우선순위는 직업창출(Ⅰ) > 취업관문(Ⅱ) > 부가우대(Ⅲ) 입니다.
    * Ⅰ 직업창출형 (자격 자체가 직업):
      - Ⅰ-1 (면허전환형): 자격 취득 → 행정청 면허 발급으로 평생 직업·신분 부여 (「○○ 면허를 받아야 한다」)
      - Ⅰ-2 (개업창업형): 자격자 본인이 단독으로 직무 수행·서명 가능 → 1인 사업 가능
    * Ⅱ 취업관문형 (자격이 채용 요건):
      - Ⅱ-1 (등록필수형): 사업체 등록·허가 시 자격자 보유 의무 (「등록기준」, 「허가기준」)
    ...
    * Ⅲ 부가우대형 (입직 후 효용):
      - Ⅲ-1 (부가우대-시험면제): 다른 자격·면허·임용시험에서 시험과목 면제 (「시험의 면제」)
      - Ⅲ-2 (부가우대-인사): 공무원·군무원·근로자의 채용·보수·평정·승진 우대 (「가산점」, 「경력경쟁채용」, 「수당」)
      - Ⅲ-3 (부가우대-위촉·자문): 위원회·심의위원·시험위원 등 자문성 위촉 자격 (「위원의 자격」, 「위촉」)
    * Ⅳ 제외 (분류 외):
      - Ⅳ-0 (제외): 타 조문 통합·삭제·이관, 정의 조항, 우대 외 사항, 직접 관련 없음

    #### 중대재해처벌법 대상 여부 (안전 책임 연계)
    해당 조문이 「중대재해처벌법」과 연계되는 안전관리 의무를 자격자에게 부여하는지 판단하세요.
    아래 영역에 해당하는 '의무고용·직무권한부여(선임·배치)' 조항이면 "대상"으로 판단합니다.
      - 중대산업재해: 종사자 보호 관련 안전관리자·보건관리자 선임 등 (예: 산업안전보건법)
      - 중대시민재해: 시설물·시민 보호, 환경·에너지 관련 안전관리 (예: 승강기안전관리법, 시설물안전법, 대기·수질환경)
      - 고압가스 안전관리: 고압가스안전관리법, 도시가스사업법, 액화석유가스법 등 가스 안전관리자
      - 검사대상기기 조종자·에너지관리자: 에너지이용합리화법 등 검사대상기기 운전·관리
    판단 원칙: 단순 인사우대·시험면제(D·E 유형)는 "비대상". 안전관리자로서 의무적으로 선임·배치되어
    사고 시 책임이 따르는 경우만 "대상". 애매하면 "비대상"으로 하고 검토필요에 표시하세요.
    ...
    ### ⚠️ [Track1 코드 작성 필수 규칙]
    - Track1_취급유형이 "Z"(제외)이면, Track1_위험도는 반드시 "X"(해당없음)로 작성하십시오. (Z↔X는 항상 짝)
    - 자격을 직접 취급하는 조항이 없거나 단순관련/해당없음으로 판별한 법령은 취급유형 "Z" + 위험도 "X"로 작성하십시오.
    - "N/A", "없음", 빈칸 등은 절대 사용하지 마십시오. 반드시 위 코드 중 하나를 고르십시오.
    """
    ...
    try:
        ...
        # 🛡️ [Track1 정규화] AI가 N/A·없음·빈칸을 뱉어도 정식 코드로 교정 + Z↔X 짝 강제
        _t1_type = (data.get("Track1_취급유형", "") or "").strip()
        _t1_risk = (data.get("Track1_위험도", "") or "").strip()
        _INVALID = {"N/A", "n/a", "NA", "없음", "-", ""}
        if _t1_type in _INVALID:
            _t1_type = "Z"          # 취급유형 불명/해당없음 → 제외
        if _t1_type == "Z":
            _t1_risk = "X"          # Z면 위험도는 반드시 X (매트릭스 규칙)
        elif _t1_risk in _INVALID:
            _t1_risk = "N"          # 취급유형은 있는데 위험도만 빔 → 무관
        data["Track1_취급유형"] = _t1_type
        data["Track1_위험도"] = _t1_risk
